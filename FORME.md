...
The detail page resolves its photo on the server instead, through
`lib/photos.ts`. It reuses the category's search and the game's position in that
category, so a card and its detail page show the same photo, and it credits the
photographer beneath the image. Without a key — or if Unsplash is slow or down —
it falls back to the same local placeholder cover.
...
- **Money** — `lib/format.ts` renders `12.50 JOD`: two decimals, a space, the
  ISO 4217 code. Never a symbol and never a local abbreviation such as `JD`.
  The catalogue is priced in Jordanian dinar; the formatter takes the code from
  each game, so mixing currencies later needs no code change.
...
lib/
  ...
  photos.ts                      server-side photo lookup for the detail page
